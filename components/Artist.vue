  ...
  <div class="results-item">
    <div class="results-item-preview" v-if="showPreview">
      ...
      >
        ...
        <img src="~/assets/icons/favorite-filled.png" v-show="isFavorited"/> <span>Favorite</span>
      </button>
    </div>
    <div class="results-item-info">
      <div class="results-item-title-container">
        ...
        <a
          v-if="showPreview && artist.youtube_link"
          class="results-item-title-youtube"
          :href="artist.youtube_link"
          target="_blank"
        > <img src="~/assets/icons/youtube.png" alt="YouTube"/> </a>
      </div>
      ...
      <div class="results-item-tags">
        ...
        <span
          class="results-item-friday-tag results-item-appointment-tag"
          v-if="artist.preview && artist.preview !== 'No'"
        >
          <img src="~/assets/icons/preview-filter.png" alt="By Appointment"/>
          <strong>Friday Preview</strong>
          </span> <span
        class="results-item-friday-tag results-item-appointment-tag"
        v-if="artist.appointment && artist.appointment !== 'No'"
      >
          <img src="~/assets/icons/appointment-only.png" alt="By Appointment"/>
          <strong>By Appointment:&nbsp;</strong> {{ artist.appointment }}
        </span>
      </div>
      <div class="results-item-not-participating" v-if="artist.not_participating">
        <img src="~/assets/icons/calendar-close.png" alt="Unable to Participate"/> Unable to Participate
      </div>
      <address class="results-item-address" v-if="showAddress">
        {{ street }} <br/> {{ address.city + ',' + address.state_zip }} <br>
      ...
      <div class="results-item-links" v-if="showAddress">
        <a :href="mailto" v-if="artist.email_address" target="_blank">{{ artist.email_address }}</a> <br/>
        <a :href="website" v-if="artist.website" target="_blank">{{ hostname }}</a>
      </div>
      <a
        v-if="!showPreview && artist.youtube_link"
        class="results-item-youtube"
        :href="artist.youtube_link"
        target="_blank"
      > <img src="~/assets/icons/youtube.png" alt="YouTube"/> <strong>YouTube</strong> </a>
      <footer>
        Medium: <span v-for="(medium, index) in artist.mediums" class="artist-medium">
