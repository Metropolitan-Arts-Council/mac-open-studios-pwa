    ...
    <nav class="filters">
        <header class="filters-header">
            <div class="filter-buttons">
                <button v-if="useDateFilter" class="pill button-blank" :class="{blue: showDateFilter}" @click="toggleDateFilter">
                ...
                <button class="pill button-blank" :class="{blue: showMediumFilter}" @click="toggleMediumFilter">
            ...
            <h5 class="flex-col text-right filters-count">
                <em v-if="isFiltering">
                    <template v-if="checkedDates.length">
                        {{checkedDates.length}} <span>{{resultsDateWording}}</span>
                    </template>
                    <span v-if="checkedDates.length && checkedMediums.length">and</span>
                    <template v-if="checkedMediums.length">
                        {{checkedMediums.length}} <span>{{resultsMediumWording}}</span>
                    </template>
                    <span>with</span>
                </em>
        ...
        <div class="mediums-filter-list" :class="{ hide: !showMediumFilter }">
            <FiltersClose @click="showMediumFilter = false" />
            <ul class="buttons-list">
                <li v-for="medium in mediums">
                    <button
                        :class="{ active: checkedMediums.includes(medium) }"
        ...
        <div v-if="useDateFilter" class="calendar" :class="{ hide: !showDateFilter }">
            <FiltersClose @click="showDateFilter = false" />
            <h3>November</h3>
            ...
            <div class="month">
                <div v-for="day in dates" :key="day.value" class="day">
                    ...
                    <a v-else href="#" @click.prevent="toggleDate(day.value)">
                        <div class="value" :class="{active: checkedDates.includes(day.value)}">{{ day.value }}</div>
                    </a>
                </div>
            </div>
        </div>
...
import FiltersClose from './FiltersClose.vue';

export default {
    props: ['mediums', 'resultsCount', 'useDateFilter'],

    components: {FiltersClose},
    ...
    computed: {
        ...
        isFiltering() {
            return this.checkedDates.length || this.checkedMediums.length;
        },

        resultsWording() {
            return this.resultsCount === 1 ? 'Artist' : 'Artists';
        },

        resultsDateWording() {
            return this.checkedDates.length === 1 ? 'Date' : 'Dates';
        },

        resultsMediumWording() {
            return this.checkedMediums.length === 1 ? 'Medium' : 'Mediums';
        },
    },

    watch: {
        checkedDates(checked) {
            this.$emit('filterByDate', checked);
        },

        checkedMediums(checked) {
            this.$emit('filterByMedium', checked);
        },
    },

    methods: {
        toggleDateFilter() {
            this.showMediumFilter = false;
            this.showDateFilter = !this.showDateFilter;
        },

        toggleMediumFilter() {
            this.showDateFilter = false;
            this.showMediumFilter = !this.showMediumFilter;
        },

        toggleDate(date) {
            this.toggleItem('checkedDates', date);
        },

        toggleMedium(medium) {
            this.toggleItem('checkedMediums', medium);
        },

        toggleItem(key, item) {
            if (this[key].includes(item)) {
